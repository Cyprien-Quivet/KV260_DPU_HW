...
// ==============================================================
`timescale 1ns/1ps
module bd_eab1_hsc_0_CTRL_s_axi
#(parameter
...
//------------------------Local signal-------------------
    ...
    reg  [0:0]                    int_phasesH_shift0;

//------------------------Instantiation------------------
// int_hfltCoeff
bd_eab1_hsc_0_CTRL_s_axi_ram #(
    .MEM_STYLE ( "auto" ),
    .MEM_TYPE  ( "2P" ),
    .BYTES     ( 4 ),
    .DEPTH     ( 192 )
...
assign hfltCoeff_q0           = int_hfltCoeff_q0 >> (int_hfltCoeff_shift0 * 16);
...
assign int_phasesH_be1        = int_phasesH_we1 ? WSTRB : 'b0;
...
// int_hfltCoeff_shift0
always @(posedge ACLK) begin
    if (ARESET)
        int_hfltCoeff_shift0 <= 1'b0;
    else if (ACLK_EN) begin
        if (hfltCoeff_ce0)
            int_hfltCoeff_shift0 <= hfltCoeff_address0[0];
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_phasesH_shift0 <= 1'b0;
    else if (ACLK_EN) begin
        if (phasesH_ce0)
            int_phasesH_shift0 <= phasesH_address0[0];
    end
end
...
module bd_eab1_hsc_0_CTRL_s_axi_ram
#(parameter
    MEM_STYLE = "auto",
    MEM_TYPE  = "S2P",
    BYTES  = 4,
